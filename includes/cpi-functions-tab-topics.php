 ...
 *
 * @return obj Tag cloud for plugin tags.
 */
function ddw_cpi_generate_topic_links( $topic_tags_array ) {
	...
	/** Render topic tag out, using native WP functionality */
	return wp_generate_tag_cloud(
		$tags,
		apply_filters(
			'cpi_filter_topic_tag_cloud_args',
			array(
				'single_text'   => __( '%s plugin', 'cleaner-plugin-installer' ),
				'multiple_text' => __( '%s plugins', 'cleaner-plugin-installer' ),
				'separator'     => ' | ',
				'smallest'      => 14,
				'largest'       => 14,
				'unit'          => 'px',
 ...
 *
 * @return string Markup for a topic part/ tags list.
 */
function ddw_cpi_render_topic_tags( $topic_title, $topic_tags ) {

	/** Building the HTML presentation */
	$topic_part = sprintf(
		'<h3>%s</h3><p class="popular-tags">%s</p><br class="clear" />',
		esc_attr( $topic_title ),
		ddw_cpi_generate_topic_links( $topic_tags )
	);
