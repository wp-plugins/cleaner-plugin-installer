	...
	if ( ! class_exists( 'WPCore' ) ) {
		...
		echo '<div class="cpi-block cpi-tab-collections">';
			...
			echo '<div class="one-half first">';
				...
				echo '<p><a class="button-primary" href="https://wpcore.com/" target="_new"><strong>&rarr;</strong> ' . __( 'Create Collections at WPCore.com', 'cleaner-plugin-installer' ) . '</a><br />' . __( 'Create a 1 or more collections (private and/ or public)', 'cleaner-plugin-installer' ) . '</p>';
 ...
 * @param int $value
 *
 * @return int Value for plugin cards per page.
 */
 ...
 * @uses   get_user_meta()
 *
 * @return absint Value for plugin cards per page.
 */
function ddw_cpi_plugin_installer_plugin_cards_per_page() {
	...
	/** Bail early if not on plugin-install.php! */
	if ( ! is_object( $screen ) || 'plugin-install' != $screen->id ) {
